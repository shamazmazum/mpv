...
static int init(struct ao *ao)
{
    ...
    ao->format = format;
    ao->period_size = nas_data->outburst / bytes_per_sample;
...
static int get_space(struct ao *ao)
{
    ...
    pthread_mutex_lock(&nas_data->buffer_mutex);
    result = (nas_data->client_buffer_size - nas_data->client_buffer_used) / ao->sstride;
    pthread_mutex_unlock(&nas_data->buffer_mutex);
...
static int play(struct ao *ao, void **data, int samples, int flags)
{
    ...
    MP_DBG(ao, "play(%p, samples=%d, len=%d, flags=%d)\n",
           data, samples, len, flags);
...
static double get_delay(struct ao *ao)
{
    ...
    pthread_mutex_lock(&nas_data->buffer_mutex);
    result = ((double)(nas_data->client_buffer_used +
                       nas_data->server_buffer_used)) /
        (double)nas_data->buffersize;
    pthread_mutex_unlock(&nas_data->buffer_mutex);
